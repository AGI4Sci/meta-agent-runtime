...
export const RunRequestSchema = z.object({
  ...
  prompt_builder: z
    .enum(["react", "cot", "minimal", "smolagents", "swe_agent", "opencode"])
    .default("react"),
  action_parser: z.enum(["json", "xml", "function_call", "react", "opencode"]).default("json"),
  context_strategy: z
    .object({
      name: z.enum(["noop", "sliding_window", "summarization", "selective", "opencode"]),
      max_tokens: z.number().optional(),
    })
    .default({ name: "sliding_window", max_tokens: 8000 }),
  tools: z.enum(["swe", "minimal", "opencode", "custom"]).default("swe"),
  config: z
    .object({
