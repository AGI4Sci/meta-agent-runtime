...
3. `GoosePromptBuilder` 能生成 Goose 风格 prompt
4. `GooseContextStrategy` 能按预期截断历史
5. `gooseTreeTool` 支持 `depth` 且尊重 `.gitignore`
6. `gooseEditTool` 对重复匹配报错
7. `gooseShellTool` 能把非零退出码映射到 observation
